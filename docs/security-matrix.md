...
| Ruta | Proteccion |
|---|---|
| `/login` | guest, rate limit, reCAPTCHA opciónal |
| `/register` | guest, rate limit, reCAPTCHA opciónal |
| `/forgot-password` | guest, rate limit, reCAPTCHA opciónal |
...
| A09 Logging/Alerting Failures | Sí | `security_audit_logs`, eventos, sanitización | `SecurityAuditService` | ELK opciónal no implementado |
...
| Logs de auditoría | `storage/logs/audit.log`, `security_audit_logs` |
